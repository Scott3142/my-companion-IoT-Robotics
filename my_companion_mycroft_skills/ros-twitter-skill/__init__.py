...
class RosTwitter(MycroftSkill):
    ...
    def handle_new_tweets(self, message):
        ...
        if len(phrase) > 1:
            ...
            if len(ats_new_tweets) > 1:
                user_response = self.ask_yesno(phrase[0].get("at") + " has " + str(len(ats_new_tweets)) + " new tweets, would you like to hear them?")
                if user_response == 'yes':
    ...
    def _post_tweets(self, tweets):
        requests.post('http://localhost:5000/tweets', json=tweets)
